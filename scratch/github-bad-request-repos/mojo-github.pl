...
my $url = 'https://api.github.com/repositories?since=62717683';
...
my $url = 'https://api.github.com/repositories?since=62717683';
my $tx = $ghojo->ua->get( $url );

	for my $item ( $tx->res->json->@* ) {
		state $count = 0;
		say "$count " . $item->{full_name};
		last if $count++ > 9;
		}
=cut
...
=pod
	...
	my $callback = make_callback( $repo_file );
	my $args = {};

	my $result = $ghojo->get_paged_resources(
